angular.module('graph').factory('graphOverSpeedFactory', ['$rootScope', '$iService',
  ($rootScope, $iService) => {
    return {
      getDataGraphOverSpeed: (data, callback) => {
        if ($rootScope.authentication.token) {
          ...
          $iService.postByHeader(`${$iService.url()}chart_speed`, strJson, header, (res) => {
            if ($.trim(res)) {
              callback(res)
            } else {
              callback([])
