angular.module('maintenance').controller('MaintenanceWorkTimeBattController', ['$rootScope', '$scope', '$iService', 'maintenanceWorkTimeBattFactory', 'settingVehicleFactory', 'indexFactory',
  ($rootScope, $scope, $iService, maintenanceWorkTimeBattFactory, settingVehicleFactory, indexFactory) => {
    ...
    $scope.modalMaintenanceWorkTimeBATT = {}
    ...
    maintenanceWorkTimeBattFactory.getMaintenanceWorkTime(res => {
      if (res.length > 0) {
    ...
    $scope.toggleModalEdit = (data) => {
      $scope.modalMaintenanceWorkTimeBATT.editModal(data)
    }
    ...
    $scope.confirmEdit = (data) => {
      if (data.modem_id != "" && data.modem_id != null && data.modem_id !== '0' &&
        ...
        if (data.edit) {
          ...
          maintenanceWorkTimeBattFactory.editMaintenanceWorkTime(data, res => {
            if (res.success) {
              const _modem_id = $scope.dataForkliftBATT.map((_data) => (_data.modem_id))
              const _index = _modem_id.indexOf(data.modem_id)
              if (_index != -1) {
                $scope.dataForkliftBATT[_index].status = 'left'
                $scope.dataForkliftBATT[_index].lastcheck = parseFloat(data.last_check)
                $scope.dataForkliftBATT[_index].check_every = parseFloat(data.check_every)
                $scope.dataForkliftBATT[_index].duecheck = parseFloat(_duecheck)
                $scope.dataForkliftBATT[_index].left_working_hour = parseFloat(_duecheck) - parseFloat(data.last_check)
                $iService.toggleModalMessage({
                  title: $rootScope.text.page_maintenance_work_time_batt.alert_title_success,
                  detail: $rootScope.text.page_maintenance_work_time_batt.alert_detail_success
                })
              } else {
                $iService.toggleModalMessage({
                  title: $rootScope.text.page_maintenance_work_time_batt.alert_title_error,
                  detail: $rootScope.text.page_maintenance_work_time_batt.alert_detail_error
                })
              }
            } else {
              $iService.toggleModalMessage({
        ...
        $iService.toggleModalMessage({
          title: $rootScope.text.page_maintenance_work_time_batt.alert_title,
          detail: $rootScope.text.page_maintenance_work_time_batt.alert_blank
        })
      }
    }
...
]).directive('modalMaintenanceWorkTimeBatt', ['$rootScope',
  ($rootScope) => {
    return {
      templateUrl: 'maintenance/views/modal_maintenance_work_time_batt.client.view.html',
      restrict: 'E',
      transclude: true,
      replace: true,
      scope: {
...
]).directive('tableMaintenanceWorkTimeBatt', ['$rootScope', '$compile', '$iService', '$window', '$timeout',
      ...
      link: (scope, element, attrs) => {
        ...
        scope.setDataTable = (data) => {
          ...
          return element.DataTable({
            columns: [{
              title: $rootScope.text.page_maintenance_work_time_batt.table.sort,
              data: (data, type, full, meta) => (meta.row + 1),
              width: "30px",
              className: "text-right",
              responsivePriority: 1
            }, {
              title: $rootScope.text.page_maintenance_work_time_batt.table.vehicle_name,
              data: data => data.vehicle_name,
              responsivePriority: 2
            }, {
              title: $rootScope.text.page_maintenance_work_time_batt.table.mileage,
              data: data => parseFloat(parseFloat(data.now_mileage || 0) + parseFloat(data.calibrate_mileage || 0)).toFixed(3)
            }, {
              title: $rootScope.text.page_maintenance_work_time_batt.table.work_time,
              data: data => parseFloat((parseFloat(data.working_hour_norun || 0) + parseFloat(data.calibrate_working_hour || 0)) / 60).toFixed(0)
            }, {
              title: $rootScope.text.page_maintenance_work_time_batt.table.last_check,
              data: data => data.lastcheck
            }, {
              title: $rootScope.text.page_maintenance_work_time_batt.table.check_every,
              data: data => data.check_every
            }, {
              title: $rootScope.text.page_maintenance_work_time_batt.table.due_check,
              data: data => data.duecheck
            }, {
              title: $rootScope.text.page_maintenance_work_time_batt.table.status,
              data: data => scope.customizeTextWorkTime(data.status, data.left_working_hour || 0)
            }, {
              title: $rootScope.text.page_maintenance_work_time_batt.table.edit,
              data: data => {
                if ($rootScope.authentication.role == 'admin') {
                  return `<button class="btn btn-warning btn-circle btn-list" ng-click='edit(${JSON.stringify(data)})' tooltip data-placement="top" data-title="${$rootScope.text.page_maintenance_work_time_batt.edit_tooltip}"><i class="fa fa-pencil-square-o"></i></button>`
                } else {
                  return `<button class="btn btn-warning btn-circle btn-list" ng-click='edit(${JSON.stringify(data)})' tooltip data-placement="top" data-title="${$rootScope.text.page_maintenance_work_time_batt.edit_tooltip}" disabled><i class="fa fa-pencil-square-o"></i></button>`
                }
              },
            ...
            responsive: {
              details: {
                display: $.fn.dataTable.Responsive.display.modal({
                  header: row => $rootScope.text.page_maintenance_work_time_batt.title
                }),
            ...
            buttons: [
              {
                ...
                title: $rootScope.text.page_maintenance_work_time_batt.table.export_title,
                text: function(dt, button, config) {
